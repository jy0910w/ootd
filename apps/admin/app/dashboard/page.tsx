...
export default function DashboardPage() {
  return <RequireAdmin allowPartner>{(session) => <DashboardContent session={session} />}</RequireAdmin>;
}
...
function DashboardContent({ session }: { session: AdminSessionState }) {
  ...
  const [myOutfits, setMyOutfits] = useState<number | null>(null);
  ...
  const loadSnapshot = useCallback(async () => {
    ...
    try {
      if (session.user.role === "partner") {
        const mine = await api.getMyOutfits(session.accessToken);
        setMyOutfits(mine.total);
        setPendingCount(mine.items.filter((outfit) => outfit.moderationStatus === "pending").length);
        setTotalUsers(null);
        setBannedUsers(null);
      } else {
  ...
  return (
    <main>
      ...
      <section className="card">
        <h2>營運儀表板</h2>
        <p>{session.user.role === "partner" ? "目前顯示你的 partner 內容概況。" : "目前為 MVP 版本，先顯示審核與使用者管理關鍵數字。"}</p>
        <div className="kpi-grid">
          ...
          <div className="kpi">
            <div className="kpi-label">{session.user.role === "partner" ? "我的穿搭數" : "使用者總數"}</div>
            <div className="kpi-value">{session.user.role === "partner" ? (myOutfits ?? "-") : (totalUsers ?? (session.user.role === "admin" ? "-" : "N/A"))}</div>
          </div>
          <div className="kpi">
